...
const Conges = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('/api/conges/accepter', {
        headers: {
  ...
  return (
    <>
              <h1 className="ml-6 font-semibold text-[3rem] dark:text-tunisys-100 text-tunisys-100 text-center mt-3">Conges et Absences</h1>
              {demande.length <= 0 ? (
  <p  className="text-center text-lg mt-9">Aucune demande est accepte pour le moment.</p>
) : (
  demande.map((item) => (
